        ...
        with torch.no_grad():
            ...
            for domain_name in self.domains:
                for content_name in self.contents:
                    domains = torch.nn.functional.one_hot(self.domain_dict[domain_name], num_classes=len(self.domains)).repeat(self.train_batch[0].shape[0], 1)
                    contents = torch.nn.functional.one_hot(self.content_dict[content_name], num_classes=len(self.contents)).repeat(self.train_batch[0].shape[0], 1)
