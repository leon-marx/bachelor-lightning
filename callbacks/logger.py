...
class Logger(Callback):
    ...
    def log_umap(self, trainer, pl_module):
        with torch.no_grad():
            ...
            plt.close(fig)
            fig = plt.figure(figsize=(10, 8))
            plt.scatter(embedding[:, 0], embedding[:, 1], c=latent_domains, cmap='Spectral', s=5)
            plt.gca().set_aspect('equal', 'datalim')
            cbar = plt.colorbar(boundaries=np.arange(len(self.domains)+1)-0.5)
            cbar.set_ticks(np.arange(len(self.domains)))
            cbar.ax.set_yticklabels(list(self.domain_dict.keys()))
            ...
            plt.close(fig)
            fig = plt.figure(figsize=(10, 8))
            plt.scatter(embedding[:, 0], embedding[:, 1], c=latent_contents, cmap='Spectral', s=5)
            plt.gca().set_aspect('equal', 'datalim')
            cbar = plt.colorbar(boundaries=np.arange(len(self.contents)+1)-0.5)
            cbar.set_ticks(np.arange(len(self.contents)))
            cbar.ax.set_yticklabels(list(self.content_dict.keys()))
            ...
            plt.scatter(embedding[:, 0], embedding[:, 1], c=torch.zeros(normal.shape[0]), cmap='Spectral', s=5)
            plt.scatter(normal_embedding[:, 0], normal_embedding[:, 1], c=torch.ones(normal.shape[0]), cmap='Spectral', s=5)
            plt.gca().set_aspect('equal', 'datalim')
