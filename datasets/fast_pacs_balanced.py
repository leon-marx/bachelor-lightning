...
class BalancedPACSDataModule(pl.LightningDataModule):
    ...
    def train_dataloader(self):
        return Loader(self.train_path, batch_size=self.batch_size, num_workers=self.num_workers, order=self.order_dict[True], pipelin=self.pipeline)
    
    def val_dataloader(self):
        return Loader(self.val_path, batch_size=self.batch_size, num_workers=self.num_workers, order=self.order_dict[self.shuffle_all], pipelin=self.pipeline)
    
    def test_dataloader(self):
        return Loader(self.test_path, batch_size=self.batch_size, num_workers=self.num_workers, order=self.order_dict[self.shuffle_all], pipelin=self.pipeline)
